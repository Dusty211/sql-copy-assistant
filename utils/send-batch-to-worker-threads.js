...
async function sendBatchToWorkerThreads(tableName, currentDataOrig) {
    ...
    if (len <= 4) {
        ...
        for (let i = 0; i < 3; i++) {
            promises.push(getWorkerPromise(tableName, [...currentData.splice(0, quarterLength)]))
        }
        promises.push(getWorkerPromise(tableName, currentData)) // last quarter + remainder
