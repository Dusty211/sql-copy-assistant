...
function SqlSafeTextCb(match, ...args) {
    ...
    const needsEscapedBackslash = groups.literalBackslash ? true : false

    const needsRemoval = groups.backspace ? true : false
    ...
    throw new Error(
        `Unexpected: toSqlSafeText() had an unexpected named group match within replaceCb().`
    )
}

function toSqlSafeText(string) {
    return typeof string === 'string' && string.length > 0
        ? string.replace(SqlSafeTextRegex, SqlSafeTextCb)
        : ''
}
